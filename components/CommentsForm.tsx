...
const CommentsForm: React.FC<{ slug: string }> = ({ slug }) => {
  ...
  const [typedStorage, setTypedStorage] = useState<TypedLocalStore<Schema>>();
  if (typeof window !== "undefined") {
    // windowを使う処理を記述
    () => setTypedStorage(new TypedLocalStore<Schema>());
  }
  ...
  const handleCommentSubmission = () => {
    ...
    if (storeData) {
      if (typedStorage) {
        typedStorage.setItem("name", name);
        typedStorage.setItem("email", email);
      }
    } else {
      if (typedStorage) {
        typedStorage.removeItem("email");
        typedStorage.removeItem("name");
      }
    }
